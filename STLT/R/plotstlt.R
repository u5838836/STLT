...
plot.stlt=function(x,main='',...){
  ...
  if (abs(gam)<0.001) {
    ...
    plot(X[1:(5400+1)],c(gompqx1,gompqx2,paretoqx),type="l",xlab="Age", ylab="qx", col='green',ylim=c(0,1),main=main)
    ...
    if (gam<0) {
      ...
      plot(X[1:(100*((floor(100*(N-thet/gam))/100)-start)+1)],c(gompqx1,gompqx2,paretoqx),type="l",col='blue',xlab='Age',ylab='qx',main=main)
      points(x,qx[1:(which(is.na(qx))[1]-1)])
      ...
      plot(X[1:(5400+1)],c(gompqx1,gompqx2,paretoqx),type="l",col='red',ylim=c(0,1),main=main)
      points(x,qx[1:(which(is.na(qx))[1]-1)])
    }
  }
}
